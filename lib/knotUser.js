...
function encrypt(text, password) {
  var cipher = crypto.createCipher(CRYPT_ALGORITHM, password);
...
var encryptDevice = function encryptDevice(device, password) {
  if (device.user) {
    device.user.uuid = encrypt(device.uuid, password);
    device.user.token = encrypt(device.token, password);
  }
}

var decryptDevice = function decryptDevice(device, password) {
  if (device.user) {
    device.user.uuid = decrypt(device.user.uuid, password);
    device.user.token = decrypt(device.user.token, password);
  }
}
