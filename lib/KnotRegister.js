  ...
  return devices.insert(newDevice, (function(_this) {
    return function(error) {
      ...
      if (device.token == null) {
        ...
        device.uuid = uuid;
        if (device.user) {
          device.user.uuid = device.uuid;
          device.user.token = device.token;
          knotUser.encryptDevice(device, device.user.password);
          device.user.password = device.user.passwordHash;
          delete device.user.passwordHash;
        }
      }
      if (device.owner) {
