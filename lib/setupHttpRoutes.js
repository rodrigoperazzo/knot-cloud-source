...
function setupHttpRoutes(server, skynet){
  ...
  server.post('/devices/user', function (req, res) {
    skynet.sendActivity(getActivity('devices', req));
    req.merged_params.ipAddress = req.merged_params.ipAddress || getIP(req);
    if (!req.merged_params['user']['email']) {
      return res.status(500).json({ status: 401, message: 'Error: Unauthorized' });
    } else {
      knotUser.getUserByEmail(req.merged_params['user']['email'], function (err, user) {
        if (err) {
          meshbluEventEmitter.emit('user-error', { request: req.merged_params, error: err });
          return res.status(500).json({ status: 503, message: err });
        } else if (user) {
          if (!knotUser.isPasswordValid(req.merged_params['user']['password'], user[0].user.password)) {
            meshbluEventEmitter.emit('existent-user-error', { request: req.merged_params });
            return res.status(500).json({ status: 409, message:'Error: email already exists' });
          } else {
              debug("restoring", user[0]);
              meshbluEventEmitter.emit('restore', { request: req.merged_params });
          ...
          knotDeviceManager.createUUID(function (uuid) {
            register(req.merged_params, function (error, device) {
              if (error) {
                meshbluEventEmitter.emit('register-error', { request: req.merged_params, error: error.message });
                return res.status(500).json({status: 506, message: error.message });
              } else {
                meshbluEventEmitter.emit('register', { request: req.merged_params });
                res.status(201).json(device);
              }
