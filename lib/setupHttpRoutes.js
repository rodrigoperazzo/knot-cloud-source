...
function setupHttpRoutes(server, skynet){
  ...
  // bypass authorize if request was made directly to fog
  function authorizeRequestIfNotGateway(req, res, next) {
    debug('authorizeRequestIfNotGateway called with type', config.knotInstanceType);
    if (config.knotInstanceType === 'gateway') {
      debug('Gateway: authorize anyway!');
      var result = getAuthUuidAndToken(req);
      return next(result);
    } else {
  ...
  server.get('/devices', function(req, res){
    authorizeRequestIfNotGateway(req, res, function(fromDevice){
